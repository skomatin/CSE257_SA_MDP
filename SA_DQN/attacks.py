...
import Adversarial_Patch_Training_master as Adv_patch
...
def adversarial_patch(model, X, y, verbose=False, params={}):
    # if args.location == 'random':
    img_dim = (255,255)
    mask_dim = (30,30)
    exclude_box = (11,11,10,10)
    epsilon = 0.1
    iterations=100
    ...
    stride=2
    signed_grad=True
    mask_gen = Adv_patch.common.mask.MaskGenerator(img_dim, mask_dim,
                                 exclude_list=np.array([exclude_box]))
    # else:
    #     mask_gen = MaskGenerator(img_dims, tuple(args.mask_dims),
    #                              include_list=np.array([args.mask_pos + args.mask_dims]))
    attack = Adv_patch.attacks.AdversarialPatch(mask_gen, epsilon, iterations,
                              optimize_location, opt_type, stride, signed_grad)

    attack.norm = Adv_patch.attacks.norms.LInfNorm()
    objective = Adv_patch.attacks.objectives.UntargetedF0Objective()

    perturbations, errors = attack.run(model, X.data, objective, writer=Adv_patch.common.summary.SummaryWriter(), prefix='')
    return X.data + perturbations
...
def attack(model, X, attack_config, loss_func=nn.CrossEntropyLoss()):
    ...
    if method == 'cw':
        ...
        atk = fgsm
    elif method == 'adversarial_patch':
        atk = adversarial_patch
    else:
        atk = pgd
